...
const CreateProject = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Clear previews and images so local URLs are not shown
      setImages([]);
      setPreviews([]);
      // Redirect to projects list (which should fetch from backend and show only Cloudinary URLs)
      ...
      <main className="flex-1 h-full overflow-y-auto bg-gradient-to-br from-slate-800/90 to-slate-900/90 p-4 md:p-8">
        <div className="max-w-xl mx-auto bg-slate-800/90 p-8 rounded-2xl shadow-2xl min-h-[80vh]">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block mb-2 font-medium text-gray-200">Images (up to 5)</label>
              <input
                type="file"
                accept="image/*"
                multiple
