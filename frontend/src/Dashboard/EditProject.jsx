...
const EditProject = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      setPreviews((project.images || []).map(img => `${API_URL}/uploads/${img}`));
      setFetching(false);
    };
    fetchProject();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    await updateProject(id, formData, images.length > 0);
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {previews.length > 0
                  ...
                  : existingImages.map((img, i) => (
                      <img key={i} src={`${API_URL}/uploads/${img}`} alt="Existing" className="w-24 h-20 object-cover rounded border" />
                    ))}
              </div>
            </div>
            <div>
