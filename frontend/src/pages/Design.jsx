...
const Design = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      setLoading(true);
      try {
        const data = await getProjectsByCategory('design');
        setProjects(data);
      } catch (err) {
        setProjects([]);
      } finally {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500 py-12">No design projects found.</div>
      ) : (
        projects.map((project, index) => (
          <div key={project._id} className="relative">
            ...
            >
              <motion.img
                src={project.image ? `${API_URL}/uploads/${project.image}` : '/assets/placeholder.jpg'}
                alt={project.title}
            ...
            <motion.div 
              className="w-full py-12 md:py-16 px-8 md:px-16 bg-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center"
            ...
            >
              <motion.div 
                className="text-4xl font-bold text-black mb-6 md:mb-0 truncate"
                whileHover={{ x: 10 }}
              ...
              <motion.div 
                className="text-xl text-black mb-6 md:mb-0 line-clamp-3"
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                viewport={{ once: true }}
              ...
              <motion.div
                ...
                transition={{ type: "spring", stiffness: 400, damping: 10 }}
              >
                <Link to="/v" className="inline-block bg-black text-white px-8 py-3 font-medium transition-all duration-300 hover:bg-gray-800 hover:shadow-lg">
                  VIEW PROJECT
                </Link>
              </motion.div>
            </motion.div>
