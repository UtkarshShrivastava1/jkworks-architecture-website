...
const ProjectsShowcase = () => {
  ...
  // Auto-change main project every 5 seconds
  useEffect(() => {
  ...
  // Navigate to current project on Explore click
  const handleExplore = () => {
    navigate(categories[activeIndex].link);
  };

  // Navigate directly from right-side buttons
    ...
    <section className="relative w-full py-24 bg-gray-200 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          {/* Right-side clickable category buttons (navigate on click) */}
            ...
            {categories.map((category, index) => (
              <div 
                ...
                className={`flex items-center p-4 rounded-lg cursor-pointer transition-all ${
                  activeIndex === index ? 'bg-[#e9e6e0]' : 'bg-white hover:bg-gray-100'
                }`}
              >
          ...
          {categories.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all ${
                activeIndex === index ? 'bg-[#b39069]' : 'bg-[#2d2a2a]'
              }`}
